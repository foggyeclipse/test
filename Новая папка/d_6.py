...
def convert_to_degrees(vectors):
    ...
    angles = [(angle + 360) % 360 for angle in angles]  # Убедитесь, что углы положительные
    return angles
...
intersection_mask1 = np.zeros((height, width), dtype=np.uint8)
intersection_mask1 = cv2.bitwise_or(radius_background_mask, purple_line_mask)

# Отображаем объединенную маску дуги
cv2.imshow('Combined Arc Mask', intersection_mask1)
cv2.waitKey(0)
