...
orange_point_mask = np.zeros((height, width), dtype=np.uint8)
            ...
            for point in contour_inter[:, 0, :]:
                if all(int(point_distance(point, dp)) > min_distance for dp in unique_points):
                    ...
                    if abs(distance_to_center - central_radius) <= 5:  # Увеличено значение для более точного попадания
                        ...
                        cv2.circle(orange_point_mask, tuple(point), 5, 255, -1)  # Оранжевый круг на маске

# Создаем финальную маску для области между фиолетовыми линиями и оранжевыми точками
intersection_mask1 = cv2.bitwise_and(purple_line_mask, orange_point_mask)

# Показываем и сохраняем результат
cv2.imshow('Intersection Mask 1', intersection_mask1)
cv2.imwrite('intersection_mask1.png', intersection_mask1)
cv2.waitKey(0)
