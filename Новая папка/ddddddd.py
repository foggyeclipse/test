...
def distance1(p1, p2):
    print(p1)
    return int(np.sqrt((int(p1[0]) - int(p2[0])) ** 2 + (int(p1[1]) - int(p2[1])) ** 2))
            ...
            if farthest_point is not None:
                ...
                for contour_inter in contours_intersection:
                    for point in contour_inter[:, 0, :]:
                        if all(int(distance1(point, dp)) > min_distance for dp in drawn_points):
                            print(type(point))
                            print(drawn_points)
                            cv2.circle(rgba_image, tuple(point), 5, (255, 0, 255), -1)
                            drawn_points.append(point)

                        # print(f"Found {len(contours_intersection)} contours.")
                        # for i, contour in enumerate(contours_intersection):
